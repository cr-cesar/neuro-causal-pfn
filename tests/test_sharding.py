...
def test_finalize_selects_winner_from_history(tmp_path):
    ...
    for backbone, pehe in (("cnn", 0.20), ("resnet18", 0.10)):
        for seed in (0, 1):
            rows.append({"label": f"E3[backbone={backbone}]", "seed": seed,
    ...
    context = {}
    out = runner.finalize_experiment("E3", context, out_root=str(tmp_path))
    assert out["winner"]["label"] == "E3[backbone=resnet18]"   # lower root-PEHE
