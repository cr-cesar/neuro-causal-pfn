...
- NIHSS and time-to-scan are absent from the real Giles-derived cohort, so E5a
...
REGISTRY: List[Experiment] = [
    ...
    Experiment(
        eid="E7", arm="A", title="Encoder backbone",
    ...
    Experiment(
        eid="E3", arm="A", title="Latent dimensionality",
        what_changes="latent dimensionality (symmetric + asymmetric)",
        baseline="E7-winner backbone + E2 loss, at 50+50; N(0,I) prior",
        variant="symmetric {25+25, 50+50, 75+75, 100+100} and asymmetric "
                "{75+25, 60+40, 40+60, 25+75}",
        tiers=("T2", "T3", "T4"), entry="e3_sweep", depends_on=("E7",),
    ...
    Experiment(
        ...
        tiers=("T2", "T3"), entry="vae", depends_on=("E7",),
    ...
    # ----------------------------- Arm D ---------------------------------- #
    Experiment(
        eid="E10b", arm="D", title="Objective (masked modelling)",
        ...
        tiers=("T1", "T2", "T4"), entry="mae", depends_on=("E7",),
    ...
    # ----------------------------- Arm E ---------------------------------- #
    Experiment(
        eid="E8a", arm="E", title="Prior (factorized -> conditional p(z|pa_x))",
    ...
    # --------------------------- Cross-arm audit -------------------------- #
    Experiment(
        eid="E11", arm="All", title="Equity audit across best configs",
...
# E1 -> E2 (loss) -> E7 (backbone; winner defaults) -> E3 (dimensionality) ->
# E4 (ARD). E5-E6 parallel after E7. E9 after E6.
ARM_A_ORDER: Tuple[str, ...] = (
    "E1", "E2", "E7", "E3", "E4", "E5a", "E6", "E9a", "E9b",
)
...
def dependency_order(ids: Optional[List[str]] = None) -> List[str]:
    ...
    assumed already available from a prior run, e.g. the E7 winner)."""
